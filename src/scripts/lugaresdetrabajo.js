	function load(page){
		...
		$('#dataUpdateLugarDeTrabajo').on('show.bs.modal', function (event){
		  var button = $(event.relatedTarget) // Botón que activó el modal
			var titulo = button.data('titulo')
			var id = button.data('id')
			var trabajonombre = button.data('trabajonombre')
			var trabajosec = button.data('trabajosec')

		  var modal = $(this)
		  modal.find('.modal-title').text(titulo)
			modal.find('.modal-body #trabajoid').val(id)
			modal.find('.modal-body #trabajonombre').val(trabajonombre)
			modal.find('.modal-body #trabajosec').val(trabajosec)
		  $('.alert').hide();//Oculto alert
		})

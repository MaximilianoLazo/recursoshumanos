...
if(!isset($_SESSION["usuario_id"])){
  header("../parametros/login/index.php");
}
...
class ParametroController{
    ...
    public function Index(){
        require_once 'view/parametro.php';
    }
    public function GuardarF(){
      $par = new parametro();
      
      $par->parametrodesde = $_REQUEST['hasta'];
      $par->parametrohasta = $_REQUEST['desde'];
       
          $par->parametromonto = $_REQUEST['monto'];
          $par->parametroporcentaje = $_REQUEST['porcentaje'];
    
      $par->parametrocodtipo = $_REQUEST['cbonovedadtipo'];
      $par->parametronombre = $_REQUEST['paranombre'];
           
      //  $par->parametroid > 0
      //      ? $this->model->ActualizarF($par)
      //      : $this->model->RegistrarF($par);
      $this->model->RegistrarF($par);
      header("Location: index.php?c=parametro&a=ParametroAnti");
    }

    public function DeshabilitarF(){
      $par = new parametro();

      $par->parametroid1 = $_REQUEST['parametroid'];
      $par->parametroactivo = 0;
      var_dump($_REQUEST['parametroid']);
      var_dump($_par->parametroactivo);

      $this->model->DeshabilitarFer($par);

      header("Location: index.php");
    }

    public function ParametroAnti(){
      require_once 'view/parametro.php';
    }

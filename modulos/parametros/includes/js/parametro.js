	...
	//------------- llena formulario de Vista --------
		...
		$('#dataUpdateFeriado').on('show.bs.modal', function (event){
		  var button = $(event.relatedTarget) // Botón que activó el modal
			var titulo = button.data('titulo')
			var id = button.data('id')
		  ...
		  modal.find('.modal-title').text(titulo)
			...
			modal.find('.modal-body #feriadofecha').val(ferfecha)
			modal.find('.modal-body #feriadoobservacion').val(ferobservacion)
		  $('.alert').hide();//Oculto alert
		})
		//------ llena formulario para eliminacion ------
		$('#dataDeleteParametro').on('show.bs.modal', function (event){
		  var button = $(event.relatedTarget) // Botón que activó el modal
		  var titulo = button.data('titulo')
		  var id1 = button.data('paramid') // Extraer la información de atributos de datos

			var modal = $(this)
			modal.find('.modal-title').text(titulo)
		  modal.find('.modal-body #parametroid').val(id1)
		})

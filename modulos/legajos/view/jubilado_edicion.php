					...
					<div class="wizard-content">
						<form class="tab-wizard wizard-circle wizard" id="" action="?c=Legajo&a=Index" method="post" enctype="multipart/form-data">
							<!-- Step 1 -->
													...
													<tr>
														...
														<td class="dt-right">
																	<button type="button"
																		...
																		data-dninro="<?php echo $fam->leghijo_nrodocto; ?>"
																		data-ape="<?php echo $fam->leghijo_apellido; ?>"
																		data-nom="<?php echo $fam->leghijo_nombres; ?>"
																		data-fec="<?php echo $fam->leghijo_fecnato; ?>"
																		data-cel="<?php echo $fam->leghijo_celular; ?>"
								...
								<?php if(!isset($liquidacion_datos->legempleado_numerol)){ ?>
								<h5>Categoría</h5>
								<section>
									...
									<div class="row">
										<div class="col-md-12">
											<div class="form-group">
												...
												class="btn btn-outline-secondary"
																	data-toggle="modal"
																	data-target="#CategoriaAlta"
																	data-titulo="<?php echo "AGREGAR NUEVO CATEGORIA"; ?>"
																	data-pid="<?php echo $jub; ?>"
																	data-nom="<?php echo $jubilado_datos->legajo_apellido .','.$jubilado_datos->legajo_nombres; ?>"
																	>
																ALTA CATEGORIA
												</button>
												<?php $fecha_jubilacion = $this->model->ListarFechaJubilacion($jub); 	?>
									...
									<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
										<div class="clearfix mb-20">
											<div class="pull-left">
												<h5 class="text-blue">CATEGORIAS </h5>
											</div>
										</div>
										...
										<div class="row">
											<table class="data-table table-bordered stripe hover nowrap">
												...
												<tbody>
												<?php $categoria = $this->model->ListarCategoriaAnual($jub);
											foreach($categoria as $row):
												$fechai= new datetime($row->fecha_inicio);
												$fei=$fechai->format('d/m/Y');
												$fechaf= new datetime($row->fecha_fin);
												$fef=$fechaf->format('d/m/Y');
											?>
											<tr>

											<td class="dt-right"><?php echo $row->categoria;?></td>
											 <td class="dt-right"><?php echo $fei;  ?></td>
											 <td class="dt-right"><?php echo $fef;  ?></td>
												<td class="dt-right">
													<!--
													<button type="button"
																			class="btn btn-danger"
																			data-toggle="modal"
																			data-target="#CategoriaEliminar"
																			data-titulo="<?php //echo "BORRAR CATEGORIA"; ?>"
																			data-cat="<?php //echo $row->categoria; ?>"
																			data-jubid="<?php //echo $row->legajo_id; ?>"
																			data-nombre="<?php //echo $jubilado_datos->legajo_apellido .','.$jubilado_datos->legajo_nombres; ?>"
																			>
																			<i class='icon-copy fa fa-trash'></i>
															</button>
						...
						<section>
							...
							<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											<div align="right">
												<a class="btn btn-outline-secondary" href="index.php?c=legajo&a=HaberEdicion&id=<?php echo $jub; ?>&seccion=5">Editar Cálculo Haber</a>

												<button type="button"
												class="btn btn-outline-secondary"
																	data-toggle="modal"
																	data-target="#GenerarHaberInicial"
																	data-titulo="<?php echo "GENERAR LIQUIDACION DE JUBILADO "; ?>"
																	data-pid="<?php echo $jub; ?>"
																	data-nom="<?php echo $jubilado_datos->legajo_apellido .','.$jubilado_datos->legajo_nombres; ?>"
																	data-imp="<?php echo $jubilado_hab->importe_haber; ?>"
																	>
																Generar Liquidación Inicial
												</button>
							...
							<div class="pd-10 bg-white border-radius-4 box-shadow mb-30">
								<div class="clearfix mb-20">
									<div class="pull-left">
										<h5 class="text-blue">Prorrateo para cálculo del Haber Jubilatorio </h5>
									</div>
								</div>
								<div class="row">
									<table class="data-table table-bordered stripe hover nowrap">
										...
										<tbody>
										 	<?php $tot=0;$haber_anual = $this->model->ListarHaberAnual($jub);
											foreach($haber_anual as $rowhaber):
										 	?>
											<tr>
											<td class="dt-right"><?php echo $rowhaber->anio;?></td>
											 <td class="dt-right"><?php echo $rowhaber->importe_haber_anual; $tot=$tot+$rowhaber->importe_haber_anual ?></td>
											</tr>
										<?php endforeach; ?>
										<tr>
										...
										<tr>
											<td class="dt-right"><?php echo "/ 120";?></td>
											<td class="dt-right"><?php $tot120=$tot/120;echo number_format($tot120,2); ?></td>
										</tr>
										<tr>
											<td class="dt-right"><?php echo "% 82";?></td>
											<td class="dt-right"><?php $tot82=$tot120*82/100;echo number_format($tot82,2);?></td>
										</tr>
										</tbody>

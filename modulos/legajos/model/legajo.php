...
public function JubiladoObtenerEdicionHaber($leg){
	try{
		...
		$sql = 'SELECT *
				FROM legajos a,liquidacion_jub_anual b
				WHERE b.legajo_id= ?
...
public function ListarJubiladosHaber(){
	try{

		$sql = "SELECT *
						FROM legajos a,documentos_tipo b,sexos c,haber_jubilado d
						WHERE empresa_id=2
...
public function HaberImporteModiExe($data){
	try{
		$importe_ant=$data->importe_haber * ($data->ant/100);
		$sql = 'UPDATE liquidacion_jub_mensual
							 SET importe_haber_mensual = ?,
							 	importe_antiguedad=?,
								categoria_id=?,
								importe_arreglado=1
							 WHERE haber_jub_id = ?';
		...
		$dec->bindValue(2, $importe_ant, PDO::PARAM_STR);
		$dec->bindValue(3, $data->categoria, PDO::PARAM_STR);
		$dec->bindValue(4, $data->id_haberme, PDO::PARAM_STR);

		$dec->execute();

		$sql = 'UPDATE liquidacion_jub_anual
		SET importe_haber_anual=(SELECT SUM(importe_haber_mensual)
		FROM liquidacion_jub_mensual
		WHERE legajo_id=?
		AND anio=?
		GROUP BY anio)
		...
		$dec = $this->cn->prepare($sql);
		$dec->bindValue(1, $data->legajo, PDO::PARAM_STR);
		$dec->bindValue(2, $data->anio1, PDO::PARAM_STR);
		$dec->bindValue(3, $data->legajo, PDO::PARAM_STR);
		$dec->bindValue(4, $data->anio1, PDO::PARAM_STR);


		$dec->execute();

		$sql = 'UPDATE haber_jubilado
		SET  importe_suma=(SELECT SUM(importe_haber_anual) FROM liquidacion_jub_anual
		WHERE legajo_id=?
		GROUP BY legajo_id),
		importe_haber=(SELECT (SUM(importe_haber_anual)/120)*0.82 FROM liquidacion_jub_anual
		WHERE legajo_id=?
		GROUP BY legajo_id)
		WHERE legajo_id=?
		';
		...
		$dec->bindValue(1, $data->legajo, PDO::PARAM_STR);
		$dec->bindValue(2, $data->legajo, PDO::PARAM_STR);
		$dec->bindValue(3, $data->legajo, PDO::PARAM_STR);
...
public function GenerarHaberInicialExe($data){
	try{
		...
		$importe_1=$data->importe / 120;
		$importe_2=$importe_1 * 82 / 100;
		...
		$sql = 'INSERT INTO legajo_haberjub(legajo_id,legajohaber_importe,
							periodo_id
							 )
							VALUES(:cat,
							:legajo2,
							:periodo)';
		$dec = $this->cn->prepare($sql);
		$dec->bindValue(':cat', $data->legajo_id, PDO::PARAM_STR);
		$dec->bindValue(':legajo2', $importe_2, PDO::PARAM_STR);
		$dec->bindValue(':periodo', 0, PDO::PARAM_STR);

		$dec->execute();

		$sql = 'INSERT INTO legajo_banco(legajo_id,numero_cuenta,sucursal,legajo_nrodocto,legajo_apellido,legajo_nombres,activo_n)
		(SELECT legajo_id,0,0,legajo_nrodocto,legajo_apellido,legajo_nombres,1
		FROM legajos
		WHERE legajo_id=?
		)';
...
public function LegajoDatosJubiladoNuevo($data){
	try{
		...
		for($i=1; $i< $cantidad; $i++){
			...
			$sql = 'INSERT INTO liquidacion_jub_anual(importe_haber_anual,anio,legajo_id)
			VALUES(:monto,
							:anioo,
							:legajo)';
		...
		for($i=1; $i< 121; $i++){
				...
				$sql = 'INSERT INTO liquidacion_jub_mensual (legajo_id,anio,mes,importe_haber_mensual)
																		VALUES(:legajo,
																						:anioo,
																						:mes,
																						0)';
...
public function LegajoDatosHaberModiExe($data){
	try{
		...
		for ($i = 1; $i < 120; $i++){
			$sql = 'INSERT INTO liquidacion_jub_mensual (legajo_id,
																				 anio,
																				 importe_haber_anual)
																 	VALUES(:legajo,
																			  	:fecha,
		...
		$sql = 'INSERT INTO liquidacion_jub_mensual (importe_haber_mensual,
																			 anio,
																			 mes,
																		   legajo_id)
				...
				$dec->bindValue(1, $data->legajo_id, PDO::PARAM_STR);
				$dec->execute();
		$sql='INSERT INTO liquidacion_jub_anual (importe_haber_anual,anio,legajo_id)(SELECT SUM(liquidacion_importe) importe,LEFT(periodo_nombre,4) anio_jub,legempleado_numerol
				FROM liquidaciones a, periodos b
				WHERE liqcodtipo_id=1
				AND legempleado_numerol=?
				AND a.periodo_id=b.periodo_id
...
public function CategoriaEliminarExe($data){
	try{
		...
		$dec->bindValue(2, $data->categoria_id, PDO::PARAM_STR);
		  ...
		  $sql='UPDATE liquidacion_jub_anual a
		  SET  importe_haber_anual=(SELECT SUM(importe_haber_mensual) FROM liquidacion_jub_mensual
		  WHERE legajo_id=?
		  AND a.anio=anio
	...
	try{
  	...
  	$dec->bindValue(':nombres', $data->legapoderado_nombres, PDO::PARAM_STR);
   	...
   	$dec->bindValue(':fec', $data->legapoderado_fecnac, PDO::PARAM_STR);
		...
		$dec->bindValue(':dir', $data->legapoderado_direccion, PDO::PARAM_STR);
  	$dec->bindValue(':cel', $data->legapoderado_celular, PDO::PARAM_STR);
		$dec->bindValue(':fecing', $data->legapoderado_fecjub, PDO::PARAM_STR);
		$dec->bindValue(':fecegr', '0000-00-00', PDO::PARAM_STR);
...
public function ObtenerTipoLegajo($nrodocto){
	try{
		...
		$sql = 'SELECT b.legtipo_nombre FROM legajos_empleado a, legajos_tipo b WHERE a.legtipo_id = b.legtipo_id AND a.legempleado_nrodocto = ? LIMIT 1';
...
public function BajaEmpleadoExe($data){
	try{
		...
		$dec = $this->cn->prepare($sql);
		$dec->bindValue(1, $data->Empid, PDO::PARAM_STR);
		$dec->bindValue(2, $data->Fechaegreso, PDO::PARAM_STR);

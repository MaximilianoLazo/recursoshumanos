...
foreach($contratos as $nrodocto){
  ...
  <table width="100%" class="tablaempleado">
    ...
    <tbody>
      <tr>
        ...
        <td class="tablaempleado-cuerpo">'.$periodonombre.' - '.$date_start->format('d/m/Y').' - '.$date_end->format('d/m/Y').'</td>
  ...
  <table width="100%" class="tablanovedades">
    ...
    foreach($periodofechas as $periodofecha){
      ...
      $feriadodiaactual = $fichadolote->ObtenerFeriado($fechasql);
      $licenciaempleadodia = $fichadolote->ObtenerLicenciaDia($fechasql, $nrodocto);
      //---- Verificar novedades y feriados ---
      if(empty($feriadodiaactual) AND empty($licenciaempleadodia)){
        //--- No existe feriado ni licencias ---
        $novedad = "<td class='tablanovedades-cuerpoi'></td>";
      }else{
        //--- Preguntar si no existe feriado---
        if(empty($feriadodiaactual)){
  ...
  //$mpdf->AddPage('P');
  $mpdf->AddPage('P','','','','',30,15,10,12,0,5);

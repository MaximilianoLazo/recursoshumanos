<?php
require_once 'model/excepcion.php';
...
class ExcepcionController{
    ...
    public function __CONSTRUCT(){
        $this->model = new Excepcion();
    }

    public function Index(){
        require_once 'view/excepcion.php';
    }
    public function GuardarEx(){
        $exce = new Excepcion();

        $exce->Excepcionid = $_REQUEST['excepcionid'];
        $exce->Excepcionnrodocto = $_REQUEST['excepcionnrodocto'];
        $exce->Excepcionperiodo = $_REQUEST['excepcionperiodo'];
        $exce->Excepcionfecha = $_REQUEST['excepcionfecha'];
        $exce->Excepcionhorai = $_REQUEST['excepcionhorai'];
        $exce->Excepcionhoraf = $_REQUEST['excepcionhoraf'];
        $exce->Excepcionficha = $_REQUEST['excepcionficha'];
        if($exce->Excepcionficha == 1){

        }else{
          $exce->Excepcionficha = 0;
        }
        $exce->Excepcionproceso = 0;
        $exce->Excepcionactivo = 1;

        $exce->Excepcionid > 0
            ? $this->model->ActualizarEx($exce)
            : $this->model->RegistrarEx($exce);

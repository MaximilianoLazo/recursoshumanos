...
class listadoController{
    ...
    public function Index(){
        require_once 'view/listadototales.php';
    }
    public function GuardarF(){
        $fer = new listado();

        $fer->listadoid = $_REQUEST['listadoid'];
        $fer->listadofecha = $_REQUEST['listadofecha'];
        $fer->listadoobservacion = $_REQUEST['listadoobservacion'];
        $fer->listadoactivo = 1;

        $fer->listadoid > 0
            ? $this->model->ActualizarF($fer)
            : $this->model->RegistrarF($fer);
    ...
    public function DeshabilitarF(){
      ...
      $fer->listadoactivo = 0;
        ...
        $this->model->DeshabilitarFer($fer);

      header("Location: index.php");
    }

    public function LicenciaAnualOrdinariaHelp(){
        require_once 'includes/php/recibos-help.php';
    }

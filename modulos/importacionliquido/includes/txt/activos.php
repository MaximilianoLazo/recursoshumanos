<?php
  ...
  $jub_datos_bersa= $this->model->JubiladoActivoObtener();
  ...
  if($archivo == false){
   ...
   foreach ($jub_datos_bersa as $value) {
     ...
     if($jub_sueldo!=""){$haber=$haber+$jub_sueldo->cjmdg_detsueldo_importe;}
     //$jub_sueldo = $this->model->JubiladoSueldoObtener($value->cjmdg_personal_nroleg2,200);
     //if($jub_sueldo!=""){$haber=$haber+$jub_sueldo->cjmdg_detsueldo_importe;}
     //$jub_sueldo = $this->model->JubiladoSueldoObtener($value->cjmdg_personal_nroleg2,201);
     //if($jub_sueldo!=""){$haber=$haber+$jub_sueldo->cjmdg_detsueldo_importe;}
     //$jub_sueldo = $this->model->JubiladoSueldoObtener($value->cjmdg_personal_nroleg2,203);
     //if($jub_sueldo!=""){$haber=$haber+$jub_sueldo->cjmdg_detsueldo_importe;}
   
     $a=str_pad(number_format($haber,2,',',''),12,"0", STR_PAD_LEFT);
      
     $ape=str_pad($value->legajo_apellido." ".$value->legajo_nombres,31," ", STR_PAD_RIGHT);
     $valor='000102906330000000800000000';
     ...
     $valor4='000000000000000000000000000000000000000';
     $valor6='0000000000000000000000000000000000000000000000';
     $valor5='00000000000000000000000000000000000000';
...
$dest_file = 'ARLAZA-E'.'.txt';
$file_size = filesize($dest_file);
$file_name = 'ARLAZA-E'.'.txt';

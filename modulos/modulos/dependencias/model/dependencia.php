...
class Dependencia{
	...
	public function ObtenerListadoImputacion($id){
		try{

			$sql = 'SELECT * FROM legajos_contrato a, legajos_empleado b, imputaciones c WHERE a.legempleado_nrodocto = b.legempleado_nrodocto AND a.imputacion_id = c.imputacion_id AND a.imputacion_id = ? AND a.legcontrato_activo = 1 AND a.impdependencia_id = 0 AND b.legempleado_activo = 1 ORDER BY b.legempleado_apellido';
	...
	public function ObtenerListadoImputaciondos($id){
		try{

			$sql = 'SELECT * FROM legajos_contrato a, legajos_empleado b, imputaciones c WHERE a.legempleado_nrodocto = b.legempleado_nrodocto AND a.imputacion_id = c.imputacion_id AND a.imputacion_id = ? AND a.legcontrato_activo = 1 AND b.legempleado_activo = 1 ORDER BY b.legempleado_apellido';
			$stm = $this->cn->prepare($sql);
			$stm->bindValue(1, $id, PDO::PARAM_STR);
			$stm->execute();
			return $stm->fetchAll(PDO::FETCH_OBJ);
	...
	public function ObtenerDepEmpleado($id){
		try{

			$sql = 'SELECT * FROM legajos_contrato a, imputaciones_dependencias b WHERE a.impdependencia_id = b.impdependencia_id AND a.legcontrato_id = ? LIMIT 1';
			$stm = $this->cn->prepare($sql);
			$stm->bindValue(1, $id, PDO::PARAM_STR);
			$stm->execute();
	...
	public function ObtenerLTrabajo($id){
		try{

			$sql = 'SELECT * FROM lugares_trabajo WHERE trabajo_id = ? LIMIT 1';
			$stm = $this->cn->prepare($sql);
			$stm->bindValue(1, $id, PDO::PARAM_STR);
			$stm->execute();
			return $stm->fetch(PDO::FETCH_OBJ);
	...
	public function AplicarDependenciaExe($dependenciaid,$contratoid,$imputacionid){
		try{

			$sql = 'UPDATE legajos_contrato SET impdependencia_id = ? WHERE legcontrato_id = ? AND imputacion_id = ?';
			$stm = $this->cn->prepare($sql);
			$stm->bindValue(1, $dependenciaid, PDO::PARAM_STR);
			$stm->bindValue(2, $contratoid, PDO::PARAM_STR);
			$stm->bindValue(3, $imputacionid, PDO::PARAM_STR);
			$stm->execute();
	...
	public function EmpleadosXImputacion($imputacionid){
		try{
			$sql = 'SELECT COUNT(a.imputacion_id) AS imputacionc FROM legajos_contrato a, legajos_empleado b WHERE a.legempleado_nrodocto = b.legempleado_nrodocto AND a.imputacion_id = ? AND a.legtipo_id = 1 AND b.legempleado_activo = 1 AND a.legcontrato_activo = 1';
			$stm = $this->cn->prepare($sql);
	...
	public function ObtenerEmpleadosXImputaciones($id){
		try{
			$sql = 'SELECT * FROM legajos_empleado a, legajos_contrato b, sexos c, estados_civiles d, legajos_tipo e WHERE a.legempleado_nrodocto = b.legempleado_nrodocto AND a.sexo_id = c.sexo_id AND a.estcivil_id = d.estcivil_id AND b.legtipo_id = e.legtipo_id AND b.imputacion_id = ? AND b.legtipo_id = 1 AND a.legempleado_activo = 1 AND b.legcontrato_activo = 1';
			$stm = $this->cn->prepare($sql);
			$stm->bindValue(1, $id, PDO::PARAM_STR);
			$stm->execute();
	...
	public function BuscarEmpleadosXImputaciones($imputacionid, $search){
		try{
			$sql = "SELECT * FROM legajos_empleado a, legajos_contrato b, sexos c, estados_civiles d, legajos_tipo e WHERE a.legempleado_nrodocto = b.legempleado_nrodocto AND a.sexo_id = c.sexo_id AND a.estcivil_id = d.estcivil_id AND b.legtipo_id = e.legtipo_id AND b.imputacion_id = ? AND b.legtipo_id = 1 AND a.legempleado_activo = 1 AND b.legcontrato_activo = 1 AND (a.legempleado_apellido LIKE '%$search%' OR a.legempleado_nombres LIKE '%$search%')";
			$stm = $this->cn->prepare($sql);
			$stm->bindValue(1, $imputacionid, PDO::PARAM_STR);

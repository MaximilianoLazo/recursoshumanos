	function load(page){
		...
		$('#dataUpdateImputacion').on('show.bs.modal', function (event){
		  var button = $(event.relatedTarget) // Botón que activó el modal
			var titulo = button.data('titulo')
			var id = button.data('id')
			var cencodigo = button.data('cencodigo')
			var cennombre = button.data('cennombre')
			var cenimporteau = button.data('cenimporteau')
			var cencantidadau = button.data('cencantidadau')

		  var modal = $(this)
		  modal.find('.modal-title').text(titulo)
			modal.find('.modal-body #imputacionid').val(id)
			modal.find('.modal-body #imputacioncodigo').val(cencodigo)
			modal.find('.modal-body #imputacionnombre').val(cennombre)
			modal.find('.modal-body #imputacionimporteau').val(cenimporteau)
			modal.find('.modal-body #imputacioncantidadau').val(cencantidadau)
		  $('.alert').hide();//Oculto alert
		})
		//------ llena formulario para eliminacion ------
		$('#dataDeleteFeriado').on('show.bs.modal', function (event){
		  var button = $(event.relatedTarget) // Botón que activó el modal
		  var id = button.data('id') // Extraer la información de atributos de datos

			var modal = $(this)
		  modal.find('.modal-body #feriadoid').val(id)
		})

	function load(page){
		...
		$('#dataUpdateTablaDeActividades').on('show.bs.modal', function (event){
		  var button = $(event.relatedTarget) // Botón que activó el modal
			var titulo = button.data('titulo')
			var id = button.data('id')
			var impactcodigos = button.data('impactcodigos')
			var impactnombre = button.data('impactnombre')
			var impnombre = button.data('impnombre')

		  var modal = $(this)
		  modal.find('.modal-title').text(titulo)
			modal.find('.modal-body #impactividadid').val(id)
			modal.find('.modal-body #impactividadcodigos').val(impactcodigos)
			modal.find('.modal-body #impactividadnombre').val(impactnombre)

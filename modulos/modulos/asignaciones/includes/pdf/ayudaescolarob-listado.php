...
<table class="cabeza" width="100%" style="vertical-align: bottom; font-size: 10pt;">
  ...
  $datosob = $this->model->ExportarAEOBLiquidaciones($periodoid);
  $obcantidad = $this->model->OBAECantidadObtener($periodoid);
  $sueldonob = $this->model->SueldoNetoOBAEObtenerTotal($periodoid);
  $importetotal = number_format($sueldonob->importesnob, 2, ',', '.');
  $contador2022 = 0;
$html .='
  <main>
  <div>Ayuda Escolar 2022</div>
  <table border="1">
  <thead>
    <tr>
      <th>EMPLEADO</th>
    ...
    foreach($datosob as $row){
      ...
      $beneficiarioimp = $this->model->SueldoNetoOBAEObtener($empleadonrodocto, $beneficiarionrodocto, $periodoid);
...
$html .='
    <tr>
      <td colspan="4">Cantidad de Beneficiarios: '.$contador2022.'<strong></strong></td>

...
class Panel{
	...
	///////////Sentencias Nuevas///////////////
	public function ObtenerEmpxFecNac($fecha_actual){
		try{
			/*
			$sql = 'SELECT *
								FROM legajos_empleado
							 WHERE legempleado_fecnacto = ?
			...
			*/
			$sql = $this->cn->prepare("SELECT *,
																				 DATE_ADD(
																						 ...
																						 INTERVAL IF(DAYOFYEAR(legempleado_fecnacto) >= DAYOFYEAR(CURDATE()),
																								 				 YEAR(CURDATE())-YEAR(legempleado_fecnacto),
																								 			 	 YEAR(CURDATE())-YEAR(legempleado_fecnacto)+1
																					) YEAR
						 														)
																			  AS legempleado_cumple
																			FROM legajos_empleado
																		 WHERE legempleado_fecnacto IS NOT NULL
						 											 		 AND legempleado_activo = 1
																		HAVING legempleado_cumple
																	 BETWEEN CURDATE()
																	 		 AND DATE_ADD(CURDATE(), INTERVAL 15 DAY)
																	ORDER BY legempleado_cumple");
			$sql->execute();
	...
	public function SRevistaObtener($id){
		try{
	...
	public function Secretarias()
	{
		...
		$sql = $this->cn->prepare("SELECT * FROM secretarias");
		$sql->execute();
	...
	public function RelojesSeg($ip, $nodo, $codigoe, $accion){
		try{
	...
	public function PeriodoAlta($periodoactualfinal){
		try{
			$newfechaperiodoi = date("Y-m-d",strtotime($periodoactualfinal."+ 1 days"));
			$newmesperiodoi = date("m", strtotime($newfechaperiodoi));
			if($newmesperiodoi == 12){
				//
			}else{
				//
			}
